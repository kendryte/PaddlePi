...
#define		FAC_MODEL_SIZE		(730 * 1024)
#define		LD5_MODEL_SIZE		(158 * 1024)
uint8_t 	model_data[KMODEL_SIZE];
uint8_t		ld5_data[LD5_MODEL_SIZE];
uint8_t		fac_data[FAC_MODEL_SIZE];
#else
...
INCBIN(ld5, "ld5.kmodel");
INCBIN(fac, "fac.kmodel");
#endif
...
int main(void)
{
	...
	w25qxx_read_data(0xB00000, ld5_data, LD5_MODEL_SIZE, W25QXX_QUAD_FAST);
	w25qxx_read_data(0xC00000, fac_data, FAC_MODEL_SIZE, W25QXX_QUAD_FAST);
#endif
	...
	lcd_draw_string(104, 150, "face attribute", WHITE);
	...
	ai_addr.ld5_kmodel_addr = ld5_data;
	ai_addr.fac_kmodel_addr = fac_data;
