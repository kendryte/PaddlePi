...
int main(void)
{
	...
	lcd_draw_string(104, 150, "voc 20 class detection", WHITE);
    ...
    while (1)
    {
        ...
        while (g_dvp_finish_flag == 0)
            ;
        /* run obj detect */
		memset(kpu_od_image.addr, 127, kpu_od_image.pixel * kpu_od_image.width * kpu_od_image.height);
