...
static obj_info_t hand_detect_info;
...
int main(void)
{
	...
	lcd_draw_string(104, 150, "yolo hand detection", WHITE);
	...
	while (1)
    {
		...
		ai_run(&hand_detect_info);
		
        /* run key point detect */
        for (uint32_t hand_cnt = 0; hand_cnt < hand_detect_info.obj_number; hand_cnt++)
        {
            draw_edge((uint32_t *)display_image.addr, &hand_detect_info, hand_cnt, RED);
        }
        /* display result */
